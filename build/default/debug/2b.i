...
void __attribute__((picinterrupt(("high_priority"))))H_ISR(void){
    if(PIR1bits.RCIF == 1){
        ...
        int counsumed = 0;
        if(c == 'm') allowed++;
        else if(c == 'o' && allowed==1)allowed++;
        else if(c == 'd' && allowed==2)allowed++;
        else if(c == 'e' && allowed==3){allowed++;counsumed = 1;}
        if(c == '1' && allowed == 4){
            state = 0;
        }
        else if(c == '2' && allowed == 4){
            state = 1;
        }
        else if(allowed == 4 && counsumed == 0)allowed = 0;
        PIR1bits.RCIF = 0;
    }
    return;
}
